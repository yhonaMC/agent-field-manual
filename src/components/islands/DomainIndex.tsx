        ...
        return (
          <a key={r.domain} className={`row${unlocked ? "" : " locked"}`} href={unlocked ? `/manual/${r.domain}` : undefined}>
            ...
            <div className="rprog">
              {unlocked ? (
                <>
                  <div className="rmeta">{r.lessonCount} lecciones · {pct}% dominado</div>
                  <div className="bar"><span style={{ width: `${pct}%`, background: pct > 0 && pct < 70 ? "var(--accent)" : "var(--ok)" }} /></div>
                </>
              ) : (
                <div className="rmeta">{r.lessonCount} lecciones</div>
              )}
            </div>
            <div className="rstate">
              {unlocked ? <span className="go">Abrir →</span> : <span className="locktag">▢ Se abre al aprobar {prev}</span>}
            </div>
          </a>
        );
      })}
